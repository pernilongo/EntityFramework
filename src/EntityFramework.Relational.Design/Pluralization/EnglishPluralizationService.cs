...
namespace Microsoft.Data.Entity.Scaffolding.Pluralization
{
    ...
    //using Microsoft.Data.Entity.ModelConfiguration.Design.PluralizationServices;
    ...
    public sealed class EnglishPluralizationService : IPluralizationService
    {
        ...
        /// </param>
        public EnglishPluralizationService([NotNull]IEnumerable<CustomPluralizationEntry> userDictionaryEntries)
            : this()
        {
            Check.NotNull(userDictionaryEntries, nameof(userDictionaryEntries));
        ...
        private string InternalPluralize(string word)
        {
            ...
            if (PluralizationServiceUtil.TryInflectOnSuffixInWord(
                ...
                (s) => s.EndsWith("deaf", /*true, culture*/ StringComparison.OrdinalIgnoreCase) ? s : s.Remove(s.Length - 1, 1) + "ves",
                _culture,
                out newSuffixWord))
            {
                return prefixWord + newSuffixWord;
            ...
            if (suffixWord.EndsWith("y", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (suffixWord.EndsWith("o", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
                return prefixWord + suffixWord + "es";
            }

            if (suffixWord.EndsWith("x", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
                return prefixWord + suffixWord + "es";
            }
        ...
        private string InternalSingularize(string word)
        {
            ...
            if (suffixWord.EndsWith("ies", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (suffixWord.EndsWith("oes", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
                return prefixWord + suffixWord.Remove(suffixWord.Length - 2, 2);
            }

            if (suffixWord.EndsWith("ss", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
                return prefixWord + suffixWord;
            }

            if (suffixWord.EndsWith("s", /*true, culture*/ StringComparison.OrdinalIgnoreCase))
            {
        ...
        [SuppressMessage("Microsoft.Globalization", "CA1308:NormalizeStringsToUppercase")]
        private bool IsUninflective([NotNull]string word)
        {
            Check.NotEmpty(word, nameof(word));

            if (PluralizationServiceUtil.DoesWordContainSuffix(word, _uninflectiveSuffixes, _culture)
                || (!word.ToLower(/*_culture*/).Equals(word) && word.EndsWith("ese", /*true, culture*/ StringComparison.Ordinal))
