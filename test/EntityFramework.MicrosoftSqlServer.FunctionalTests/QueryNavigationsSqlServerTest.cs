...
ORDER BY [o1].[CustomerID], [o2].[CustomerID]",
                Sql);
        }
...
ORDER BY [e].[ReportsTo]",
        ...
        public override void Select_Where_Navigation_Not_Null_Deep()
        {
            base.Select_Where_Navigation_Not_Null_Deep();

            Assert.Equal(
                @"SELECT [e].[EmployeeID], [e].[City], [e].[Country], [e].[FirstName], [e].[ReportsTo], [e].[Title]
FROM [Employees] AS [e]
LEFT JOIN [Employees] AS [e.Manager] ON [e].[ReportsTo] = [e.Manager].[EmployeeID]
WHERE [e.Manager].[ReportsTo] IS NOT NULL
ORDER BY [e].[ReportsTo]",
                Sql);
        }
